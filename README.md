...
### Run (with secrets & persistence)

```bash
docker run --rm -p 5000:5000 \
  -e SECRET_KEY='replace_me' \
  -e RESUME_PASSWORD='replace_me' \  # if you protect the résumé route
  -v "$PWD/private:/app/private:ro" \ # if serving a protected resume file from /app/private
  -v "$PWD/leaderboard.json:/app/leaderboard.json" \ # persist quiz leaderboard
  --name shig-webapp \
...
> If you make the repo private, run docker login before pulling.
> 
